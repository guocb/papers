...
\abstract{
  ...
  % why interesting
  This downgrades the user experience and development efficiency against many
...
protocol command in ABNF as well, which is shown in Figure
...
The pack stream transferred to the client cannot be handled by Git directly
...
That is what user really wants to get initially.
...
stored in the Git index to form a complete tree object, and then rewrite the
commit to point to the complete tree object.
...
protocol extension, which just using \emph{dirs://} as the remote repository
URL scheme instead of \emph{ssh://}.
...
\end{verbatim}
An editor was popped up to show the top level directories of the repository
with usage, e.g.
\begin{verbatim}
...
After saving and exiting the editor, the program start the transferring.
\begin{verbatim}
...
\end{verbatim}
That is, the tree of rewritten commit \verb|b72f3ee| includes not only
the subdirectory we transferred, but also others we filtered out.

After pushing the newly created commit to share with other developers, the
example work flow was done.
