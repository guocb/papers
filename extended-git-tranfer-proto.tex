...
In standard Git protocol, the object packing can be thought as a piped command, i.e. \verb|git-object-list\|git-pack-objects|.
That is, get a list of all commits (and its associated objects) and then pack them.

Since we have the subdirectories information, we can filter out objects not in those subdirectories.
The whole process can be thought as a command as \verb|git-object-list\|filter($D_i$, $D_e$)\|git-pack-objects|.
